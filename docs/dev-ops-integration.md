...
This tutorial is for enthusiasts of full module source code deployment, on how to automatically pull source code, automatically compile, and automatically start port running through automatic commands. To achieve the most efficient upgrade system.

The test platform of this project `https://2662r3426b.vicp.fun` has been using this method since it was opened, with good results.

The tutorial can refer to the video tutorial released by Bilibili blogger `毕乐labs`: [《Open Source Xiaozhi Server xiaozhi-server Automatic Update and Latest Version MCP Endpoint Configuration Tutorial》](https://www.bilibili.com/video/BV15H37zHE7Q)
...
At this point, first execute the first command to pull the source code. This command is suitable for servers and computers on the domestic network, no need for VPN:
...
If you have already run through the entire process before, you should be familiar with the funasr model file `xiaozhi-server/models/SenseVoiceSmall/model.pt` and your private configuration file `xiaozhi-server/data/.config.yaml`.
...
Answer: Port 8001 is used in the development environment for running the frontend. If you are deploying on a server, it is not recommended to use `npm run serve` to start port 8001 to run the frontend. Instead, compile it into html files as in this tutorial, and then use nginx to manage access.
