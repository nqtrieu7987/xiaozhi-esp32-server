...
- Disadvantages: As of September 25, 2025, the default model is a Chinese model and does not support English-to-speech conversion. If it contains English, no sound will be produced. If you need to support both Chinese and English, you need to train it yourself.
...
```
Please start the command according to the actual directory of your ```tts_online_application.yaml```. When you see the following logs, it means startup is successful:
```
...
python app.py
```
Open test_page.html in the test directory, test connection and check if paddlespeech end has output logs when sending messages
