...
async def wait_for_exit() -> None:
    ...
    if sys.platform != "win32":  # Unix / macOS
        ...
        # let KeyboardInterrupt bubble up to asyncio.run to eliminate the issue of leftover normal threads causing process exit blocking
        try:
...
async def main():
    ...
    # auth_key is used for JWT authentication, such as JWT authentication for vision analysis interface, token generation for OTA interface and websocket authentication
    ...
    logger.bind(tag=TAG).info(
        "=======The above address is a WebSocket protocol address, do not access with a browser======="
    )
    logger.bind(tag=TAG).info(
        "To test WebSocket, please open test_page.html in the test directory with Google Chrome"
    )
    logger.bind(tag=TAG).info(
        "=============================================================\n"
    )
